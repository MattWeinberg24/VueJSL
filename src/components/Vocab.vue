<template>
    ...
    <div id="vocab-io">
        ...
        <h2 id="result">{{isCorrect ? "はい":"いいえ"}}</h2>
        <h2 id="correct-answers">{{correctAnswers}}</h2>
    </div>
...
export default {
    ...
    methods: {
        randomWord() {
            ...
            if (this.unusedWords.length > 0 && (this.reviewWords.length == 0 || this.count < REVIEW_COUNT)) {
                let n = Math.floor(Math.random() * this.unusedWords.length);
                this.currentWord = this.unusedWords[n];
            }
            //choose from review words
            else if (this.reviewWords.length > 0 && (this.unusedWords.length == 0 || this.count == REVIEW_COUNT)) {
            ...
            //reset the pool of words
                ...
                //TODO: IMPLEMENT RESET AFTER ALL WORDS DONE
            }
            ...
            if(answer != ""){
                if (this.correct(answer)){
                    ...
                    if (this.count <= this.REVIEW_COUNT) { //if picked from unusedWords
                        let i = this.unusedWords.indexOf(this.currentWord);
                        //EMIT TO MOVE FROM UNUSED TO MASTERED  
                    }
                    else { //if picked from review words
                        let i = this.reviewWords.indexOf(this.currentWord);
                        //EMIT TO MOVE FROM REVIEW TO UNUSED
                        count = 0;
                    }
                    ...
                    if (this.count <= this.REVIEW_COUNT) { //if picked from unusedWords
                        var i = this.unusedWords.indexOf(this.currentWord);
                        //EMIT TO MOVE FROM UNUSED TO REVIEW
                    }
